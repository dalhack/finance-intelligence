...
@pytest.mark.asyncio
async def test_migration_019_catalog_and_immutability():
    """Verify Migration 019 catalog properties, FORCE RLS, role grants, schema version 3.0.0, and worker denial."""
    ...
    async with OwnerSession() as db_owner:
        ...
        # Seed organization
        org_id = uuid4()
        org = Organization(id=org_id, name="Cat Org 019", slug=f"cat19-{org_id.hex[:6]}")
        db_owner.add(org)
