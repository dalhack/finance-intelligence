...
async def test_migration_031_catalog_and_security():
    ...
    owner_url = os.environ.get(
        "TEST_OWNER_DATABASE_URL", "postgresql+asyncpg://postgres:postgres@localhost:5433/finance_intelligence_test"
    )
    ...
    async with OwnerSession() as owner_db:
        ...
        idx_res = await owner_db.execute(
            text(
        ...
        role_res = await owner_db.execute(
            text("SELECT rolcanlogin, rolsuper, rolbypassrls FROM pg_roles WHERE rolname = 'db_analysis_claim_owner';")
        )
        ...
        broad_pols = await owner_db.execute(
            text(
        ...
        narrow_pols = await owner_db.execute(
            text(
        ...
        acl_res = await owner_db.execute(
            text(
        ...
        for r in acl_res.fetchall():
            assert r.unauth_exec is False, (
                f"Unauthorized role db_ingestion_worker should not have EXECUTE on {r.proname}"
            )
    ...
    async with ApiSession() as api_db:
        ...
        for bad_id in invalid_worker_ids:
            ...
            assert any(
                k in str(exc_info.value)
                for k in ("CRITICAL_SECURITY_VIOLATION", "CharacterNotInRepertoireError", "invalid byte sequence")
            )
    ...
    try:
        ...
        async with ApiSession() as api_db:
            ...
            w1_res = await api_db.execute(
                text("SELECT public.renew_analysis_job_lease(:jid, :tok, 'w');"), {"jid": job_id, "tok": uuid.uuid4()}
            )
            assert w1_res.scalar() is False
            await api_db.rollback()

            # Valid 100-char worker ID
            w100_res = await api_db.execute(
                text("SELECT public.renew_analysis_job_lease(:jid, :tok, :w);"),
                {"jid": job_id, "tok": uuid.uuid4(), "w": "a" * 100},
            )
            ...
            async with OwnerSession() as owner_db:
                ...
                for r in org_rows.fetchall():
                    ...
                    await owner_db.execute(
                        text("DELETE FROM public.analysis_jobs WHERE status = 'RECEIVED' AND id != :jid;"),
                        {"jid": job_id},
                    )
                await owner_db.execute(
                    text("SELECT set_config('app.current_organization_id', :oid, true);"), {"oid": str(org_id)}
            ...
            claim_res = await api_db.execute(
                text("SELECT job_id, organization_id, claim_token FROM public.claim_next_analysis_job('worker-031-a');")
            )
        ...
        async with ApiSession() as api_db:
            conc_res = await api_db.execute(
                text("SELECT job_id FROM public.claim_next_analysis_job('worker-031-concurrent');")
            )
        ...
        async with OwnerSession() as owner_db:
            ...
            job_res = await owner_db.execute(
                text(
                    "SELECT status, locked_by, claim_token, recovery_count FROM public.analysis_jobs WHERE id = :jid;"
                ),
                {"jid": job_id},
            )
        ...
        async with ApiSession() as api_db:
            renew_ok = await api_db.execute(
                text("SELECT public.renew_analysis_job_lease(:jid, :tok, 'worker-031-a');"),
                {"jid": job_id, "tok": claimed_token},
            )
            assert renew_ok.scalar() is True

            renew_fail_tok = await api_db.execute(
                text("SELECT public.renew_analysis_job_lease(:jid, :tok, 'worker-031-a');"),
                {"jid": job_id, "tok": uuid.uuid4()},
            )
            assert renew_fail_tok.scalar() is False

            renew_fail_wrk = await api_db.execute(
                text("SELECT public.renew_analysis_job_lease(:jid, :tok, 'wrong-worker');"),
                {"jid": job_id, "tok": claimed_token},
            )
        ...
        for non_rec_status in ("NEEDS_CLARIFICATION", "AWAITING_HUMAN_REVIEW", "EXECUTING_TOOLS", "QUALITY_GATE"):
            ...
            async with ApiSession() as api_db:
                stale_nr = await api_db.execute(
                    text("SELECT job_id FROM public.recover_next_stale_analysis_job('worker-031-test');")
                )
        ...
        async with ApiSession() as api_db:
            stale_res = await api_db.execute(
                text(
                    "SELECT job_id, organization_id, claim_token FROM public.recover_next_stale_analysis_job('worker-031-b');"
                )
            )
        ...
        async with OwnerSession() as owner_db:
            ...
            att_res = await owner_db.execute(
                text("SELECT status FROM public.analysis_attempts WHERE id = :aid;"), {"aid": att_id}
            )
            att_row = att_res.fetchone()
            assert att_row.status == "ABANDONED"

            unrel_res = await owner_db.execute(
                text("SELECT status FROM public.analysis_attempts WHERE id = :uaid;"), {"uaid": unrel_att_id}
            )
            assert unrel_res.fetchone().status == "RUNNING"

            job_res2 = await owner_db.execute(
                text("SELECT recovery_count FROM public.analysis_jobs WHERE id = :jid;"), {"jid": job_id}
            )
        ...
        async with OwnerSession() as owner_db:
            ...
            await owner_db.execute(
                text("UPDATE public.analysis_jobs SET lease_expires_at = now() - INTERVAL '1 minute' WHERE id = :jid;"),
                {"jid": job_id},
            )
            await owner_db.commit()

        async with ApiSession() as api_db:
            stale_res2 = await api_db.execute(
                text("SELECT job_id FROM public.recover_next_stale_analysis_job('worker-031-c');")
            )
        ...
        async with OwnerSession() as owner_db:
            ...
            await owner_db.execute(
                text("DELETE FROM public.analysis_attempts WHERE analysis_job_id IN (:jid, :ujid);"),
                {"jid": job_id, "ujid": unrel_job_id},
            )
            await owner_db.execute(
                text("DELETE FROM public.analysis_jobs WHERE id IN (:jid, :ujid);"),
                {"jid": job_id, "ujid": unrel_job_id},
            )
