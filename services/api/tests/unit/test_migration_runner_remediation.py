...
def mock_config():
    return MigrationExecutionConfig(
        ...
        expected_head=ALLOWED_MIGRATION_HEAD,
...
def test_unlock_false_fails_closed(mock_config):
    ...
    with (
        ...
        patch(
            ...
            return_value=ALLOWED_MIGRATION_HEAD,
        ),
...
def test_t1_t2_t3_reset_role_called_before_phase3_alembic(mock_config):
    ...
    with (
        ...
        mock_upgrade.assert_called_once_with(ANY, ALLOWED_MIGRATION_HEAD)
...
def test_t6_t7_t8_t9_t10_remediation_invariants():
    ...
    assert heads == [ALLOWED_MIGRATION_HEAD]


def test_get_valid_graph_revisions_success():
    """get_valid_graph_revisions returns exactly the revisions in the script graph."""
...
def test_known_revisions_parity_with_active_graph():
    ...
    assert ALLOWED_MIGRATION_HEAD in KNOWN_REVISIONS
