...
async def test_engine_fencing_and_ownership_loss():
    ...
    async with OwnerSession() as owner_db:
        await owner_db.execute(
            text(
                "INSERT INTO public.organizations (id, name, slug, created_at, updated_at) VALUES (:id, 'Fencing Test Org', :slug, now(), now());"
            ),
            {"id": org_id, "slug": f"fence-org-{uuid.uuid4().hex[:8]}"},
        )
        await owner_db.execute(
            text(
                "INSERT INTO public.users (id, external_subject, identity_provider, display_name, status, created_at, updated_at) VALUES (:uid, :sub, 'firebase', 'Fence User', 'ACTIVE', now(), now());"
            ),
    ...
    try:
        ...
        async with ApiSession() as api_db:
            context = ExecutionContext(
                organization_id=org_id, user_id=user_id, role="OWNER", permissions={"analyses:run", "analyses:read"}
            )
            engine = AnalysisOrchestratorEngine(
                api_db, context, provider=DeterministicTestModelProvider(environment="development")
            )
        ...
        async with OwnerSession() as owner_db:
            ...
            await owner_db.execute(
                text("DELETE FROM public.result_datasets WHERE organization_id = :oid;"), {"oid": org_id}
            )
            await owner_db.execute(
                text("DELETE FROM public.comparison_runs WHERE organization_id = :oid;"), {"oid": org_id}
            )
            ...
            await owner_db.execute(
                text("DELETE FROM public.final_result_snapshots WHERE analysis_job_id = :jid;"), {"jid": job_id}
            )
            await owner_db.execute(
                text("DELETE FROM public.quality_gate_results WHERE analysis_job_id = :jid;"), {"jid": job_id}
            )
            await owner_db.execute(
                text("DELETE FROM public.tool_invocations WHERE analysis_job_id = :jid;"), {"jid": job_id}
            )
            await owner_db.execute(
                text("DELETE FROM public.analysis_plans WHERE analysis_job_id = :jid;"), {"jid": job_id}
            )
            await owner_db.execute(text("DELETE FROM public.analysis_jobs WHERE id = :jid;"), {"jid": job_id})
...
async def test_worker_db_role_execute_privilege():
    ...
    async with WorkerSession() as w_db:
        ...
        assert (
            ...
            or "insufficientprivilegeerror" in str(exc_info.value).lower()
        )
...
async def test_engine_fencing_toctou_prevention():
    ...
    async with OwnerSession() as owner_db:
        await owner_db.execute(
            text(
                "INSERT INTO public.organizations (id, name, slug, created_at, updated_at) VALUES (:id, 'TOCTOU Test Org', :slug, now(), now());"
            ),
            {"id": org_id, "slug": f"toctou-org-{uuid.uuid4().hex[:8]}"},
        )
        await owner_db.execute(
            text(
                "INSERT INTO public.users (id, external_subject, identity_provider, display_name, status, created_at, updated_at) VALUES (:uid, :sub, 'firebase', 'TOCTOU User', 'ACTIVE', now(), now());"
            ),
...
async def test_heartbeat_exception_cancels_engine_without_failure_persistence():
    ...
    worker = AnalysisWorker(
        worker_id="worker-hb-exc-test", heartbeat_interval=0.05, session_factory=failing_session_factory
    )
    success = await worker.process_analysis_job(job_id, org_id, claim_token)
    assert success is False
